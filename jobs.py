...
def train_linear(x_train, y_train):
    perform_hyperparameter_tuning(x_train, y_train, pipeline_type='linear', save=True)

def train_polynomial(x_train, y_train):
    perform_hyperparameter_tuning(x_train, y_train, pipeline_type='polynomial', save=True)

def train_zscore(x_train, y_train):
    x_norm, mean, sigma = zscore_normalize(x_train)
    perform_hyperparameter_tuning(x_norm, y_train, save=True)

def train_rf(x_train, y_train):
    params = perform_rf_hyperparameter_tuning(x_train, y_train, save=True)
    train_evaluate_random_forest(x_train, y_train, params=params, save=True)
