...
def preprocess_normalize_data(data, target_column):
    ...
    for entry in data:
        item = [
            ...
            if key != target_column and key != "ID"
        ]
...
def train_evaluate_polynomial_regression(x, y, params={"max_iter": 10000}):
    ...
    if pipeline is None:
        ...
        pipeline = Pipeline([
            ("poly", poly), ("scaler", scaler), ("regressor", model)
        ])
...
def zscore_normalize(x):
    ...
    - Normalizes each feature by subtracting the mean and
...
def train_evaluate_zscore_normalization(x, y, params={"max_iter": 10000}):
    ...
    - Visualizes the distribution of selected features before
...
def perform_hyperparameter_tuning(
    x_train, y_train, pipeline_type=None, save=False
):
    if pipeline_type == "polynomial":
        ...
        if stored_params is not None:
            return stored_params
        pipeline = Pipeline(
            [("scaler", StandardScaler()), ("regressor", SGDRegressor())]
        )
        param_grid = {
